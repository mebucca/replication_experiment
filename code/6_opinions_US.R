...
depvar <- c("ahead_fwealth","ahead_peduc","ahead_ambition","ahead_hardwork","ineq_toohigh","gov_redineq")

label <- c("Get ahead: family wealth","Get ahead: educated parents","Get ahead: ambition","Get ahead: hard work",
           "Income inequality too high", "Government should reduce inequality")
...
for (dv in depvar) {
  ...
  myplot <- newx %>%
    ...
    labs(
      colour = NULL,
      x = "inequality of opportunity",
      y = label[match(dv, depvar)]
    )
  
  # Apply ylim conditionally
  if (startsWith(dv, "ahead_")) {
    myplot <- myplot + ylim(0.25, 0.85)
  } else {
    myplot <- myplot + ylim(0, 0.85)
  }
